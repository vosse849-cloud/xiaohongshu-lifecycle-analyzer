(function () {
  ...
  function runAnalysis() {
    ...
    if (!observeDate) {
      showMessage("请填写正确的当前观察时间，例如：2026-05-27 17:30。", false);
      return;
    }

    try {
      analyzedRows = rawRows.rows.map(function (row, index) {
        return analyzeRow(row, observeDate, index);
      });
    ...
    card.innerHTML = [
      ...
      "<div class=\"snapshot-fields\">",
        "<div class=\"field-group\">",
          ...
          "<p class=\"hint\">选择这个时间点导出的小红书 CSV。</p>",
        "</div>",
        "<div class=\"field-group\">",
          ...
          "<p class=\"hint\">支持 2026-05-27 10:00、2026/05/27 10:00 和中文时间。</p>",
        "</div>",
      "</div>"
    ].join("");
    ...
    Promise.all(snapshots.map(function (snapshot) {
      return readFileAsArrayBuffer(snapshot.file).then(function (buffer) {
        ...
        return {
          ...
          date: snapshot.date,
          rows: parsed.rows.map(function (row, index) {
            return analyzeRow(row, snapshot.date, index);
          })
        };
      });
    }))
  ...
  function collectSnapshotInputs() {
    ...
    for (var i = 0; i < cards.length; i += 1) {
      ...
      if (!timeText || !date) {
        showMessage(label + " 的观察时间无法识别，请使用类似 2026-05-27 10:00 的格式。", false);
        return null;
      }
  ...
  function buildIntervalCompareRow(options) {
    ...
    return {
      ...
      compareType: !oldRow ? "new" : (!newRow ? "missing" : "matched"),
      hasNegativeDelta: hasNegativeDelta(deltas),
      matchRisk: options.key.indexOf("empty-title__") === 0
    };
  }
  ...
  function buildSnapshotMap(rows) {
    ...
    rows.forEach(function (row, index) {
      var key = buildSnapshotKey(row, index);
      if (!map[key]) {
        map[key] = row;
      }
    });
  ...
  function analyzeRow(row, observeDate, index) {
    ...
    var extractedTopic = extractTopicFromTitle(row["笔记标题"]);
    var topicKey = buildTopicStorageKey(row["首次发布时间"], row["笔记标题"]);
    var correction = getTopicCorrection(row["笔记标题"]);
    ...
    if (correction) {
      ...
      topicConfidence = "历史修正";
      topicSource = "correction";
      saveTopicCorrection(row["笔记标题"], legacySavedTopic.topic);
    }
    ...
    resultBody.innerHTML = rows.map(function (row) {
      return [
        ...
        "<td class=\"topic-cell " + topicCellClass(row.topicConfidence) + "\">" +
          ...
          "<span class=\"topic-tag\" data-tooltip=\"" + escapeHtml(getTopicConfidenceTooltip(row.topicConfidence)) + "\">" + escapeHtml(row.topicConfidence) + "</span>" +
        "</td>",
    ...
    compareResultBody.innerHTML = rows.map(function (row) {
      return [
        ...
        "<td class=\"title-cell\" title=\"" + escapeHtml(row.title) + "\">" + escapeHtml(row.title) +
          (row.matchRisk ? "<span class=\"topic-tag\" data-tooltip=\"标题为空，当前使用首次发布时间和行号辅助匹配，稳定性较低。\">匹配风险</span>" : "") +
        "</td>",
  ...
  function handleTopicEdit(event) {
    ...
    if (nextTopic === "待填写") {
      ...
      saveTopicCorrection(row.originalTitle, nextTopic);
      analyzedRows.forEach(function (item) {
        if (item.originalTitle === row.originalTitle) {
  ...
  function applyTopicCorrectionLibrary() {
    ...
    analyzedRows.forEach(function (row) {
      ...
      var correction = getTopicCorrection(row.originalTitle);
      if (!correction) {
        return;
      }
  ...
  function renderStatusDistribution(rows) {
    ...
    statusDistributionGrid.innerHTML = distribution.statuses.map(function (status) {
      return [
        ...
        "<strong>" + formatInteger(distribution.counts[status]) + "</strong>",
        "</article>"
      ].join("");
    }).join("");
  }
  ...
  function buildTopicStorageKey(publishedText, title) {
    return "xhs-topic::" + String(publishedText || "").trim() + "::" + String(title || "").trim();
  }

  function buildCorrectionKey(originalTitle) {
    return "xhs-topic-correction::" + normalizeOriginalTitle(originalTitle);
  }
  ...
  function getTopicCorrection(originalTitle) {
    var key = buildCorrectionKey(originalTitle);
  ...
  function saveTopicCorrection(originalTitle, topic) {
    var cleanTitle = normalizeOriginalTitle(originalTitle);
    var cleanTopic = String(topic || "").trim();
    if (!cleanTitle || !cleanTopic || cleanTopic === "待填写") {
      return;
    }

    try {
      localStorage.setItem(buildCorrectionKey(cleanTitle), JSON.stringify({

...
当前版本支持上传 `.csv` 和 `.xlsx` 两种数据文件：

- 单份分析可以上传 CSV 或 XLSX。
- 多快照对比可以上传 CSV、XLSX，也可以混合上传 CSV + XLSX。
- XLSX 默认读取第一个 Sheet。
- XLSX 读取成功后，页面会显示已读取的 Sheet 名称。
- XLSX 只是新增输入层，后续仍然复用原来的表头映射、指标计算、状态判断、多快照新增计算、选题修正库和 AI 摘要逻辑。
- 如果 XLSX 文件无法读取，页面会提示“XLSX 读取失败”，请确认文件是真正的 `.xlsx` 文件。

公司 MacBook Pro 上使用时，推荐文件命名继续保持观察时间格式，例如：
...
真实小红书 CSV / XLSX 数据不要提交到 GitHub；只使用脱敏 mock 数据做演示。
